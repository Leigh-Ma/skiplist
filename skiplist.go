...
func (sl *SkipList) Init() *SkipList {
	...
	sl.level = 1
	sl.dirtyNode = nil
	sl.elements = make(map[interface{}]*Element, 0)
...
func (sl *SkipList) Insert(v Interface) *Element {
	...
	sl.setDirtyNode(x)
	sl.elements[x.Value.ObjectId()] = x
...
func (sl *SkipList) deleteElement(e *Element, update []*Element) {
	sl.setDirtyNode(e.level[0].forward)
	delete(sl.elements, e.Value.ObjectId())
...
func (sl *SkipList) refreshDirtyRank(maxRank int) {
	if sl.dirtyNode != nil {
		rank := 0
		if sl.dirtyNode != sl.header.Next() {
			rank = sl.dirtyNode.backward.Value.Rank()
		}
		for rank < maxRank && sl.dirtyNode != nil {
			rank += 1
			sl.dirtyNode.Value.SetRank(rank)
			sl.dirtyNode = sl.dirtyNode.Next()
		}
	}
}

func (sl *SkipList) refreshDirtyToElement(e *Element) {
	if sl.dirtyNode != nil {
		rank := 0
		if sl.dirtyNode != sl.header.Next() {
			rank = sl.dirtyNode.backward.Value.Rank()
		}
		for sl.dirtyNode != nil {
			rank = rank+1
			sl.dirtyNode.Value.SetRank(rank)
			if sl.dirtyNode == e {
				break
...
func (sl *SkipList) dirtyRankPos() int{
	if sl.dirtyNode == nil {
		return sl.tail.Value.Rank()
	} else {
		return sl.dirtyNode.backward.Value.Rank()
	}
}

func (sl *SkipList) GetRangeElements(start, end int)(r []*Element) {
	if end >= sl.dirtyRankPos() {
		sl.refreshDirtyRank(end)
	}
	s := sl.GetElementByRank(start)
	for s != nil && start <= end{
		r = append(r, s)
		s = s.Next()
		start += 1
...
func (sl *SkipList) GetRankFast(v Interface) int {
	e := sl.elements[v.ObjectId()]
	if e == nil {
		return 0
	}

	if sl.isRankDirty(v) {
		sl.refreshDirtyToElement(e)
	}
...
func (sl *SkipList) setDirtyNode(e *Element) {
	if sl.dirtyNode == nil || (e != nil && e.Value.Less(sl.dirtyNode.Value)) {
		sl.dirtyNode = e
	}
}

func (sl *SkipList) isRankDirty(value Interface) bool {
	if sl.dirtyNode != nil && !value.Less(sl.dirtyNode.Value){
		return true
	}
	return false
}